...
//Github represents the link between github and the user's installation
type Github struct {
...
func ParseProjectSettings(s string) (*ProjectSettings, *AppError) {
	...
	if settings.Provider.Type != K8 && settings.Provider.Type != Demo {
		return nil, &AppError{Status: 400, Code: InvalidProviderType}
	}

	if settings.Provider.Type == K8 {
	...
	if settings.Github != nil {
		if settings.Github.LinkedBy == "" {
			return nil, &AppError{Status: 400, Code: MissingGithubScope}
		}
	}
...
func (p *Project) Validate() *AppError {
	...
	for _, name := range reservedProjectNames {
		if name == p.Name {
